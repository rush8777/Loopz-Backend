 ...
 * below.
 */
export function registerEventRoutes(app, db) {
    /** Distinct custom event names, most-frequent first - the catalog list (task brief section 3). */
    app.get("/orgs/:orgId/sites/:siteId/events", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
    ...
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
    ...
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName/timeseries", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, eventName } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
    ...
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName/properties", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, eventName } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
    ...
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName/occurrences", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, eventName } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
    ...
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName/occurrences/:occurrenceId", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, eventName, occurrenceId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const occurrence = await getEventOccurrence(db, site.id, eventName, occurrenceId);
        if (!occurrence)
            return reply.code(404).send({ error: "occurrence_not_found" });
        return reply.send(occurrence);
    });
    /** Users who performed this event, identity-resolved (task brief section 10). */
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName/users", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, eventName } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
    ...
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName/sessions", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, eventName } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
    ...
    app.get("/orgs/:orgId/sites/:siteId/events/:eventName/pages", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, eventName } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
