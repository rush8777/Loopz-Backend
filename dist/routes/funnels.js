...
async function loadFunnelInSite(db, siteId, funnelId) {
 ...
 */
async function validateStepReferences(db, siteId, steps) {
    const eventNames = [...new Set(steps.filter((s) => s.type === "event").map((s) => s.eventName))];
    for (const name of eventNames) {
        if (!(await eventExistsForSite(db, siteId, name)))
            return `Unknown event: ${name}`;
    }
    const pageIds = [...new Set(steps.filter((s) => s.type === "page").map((s) => s.pageId))];
    if (pageIds.length > 0) {
...
export function registerFunnelRoutes(app, db) {
    ...
    app.get("/orgs/:orgId/sites/:siteId/funnels", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        ...
        const [{ total }] = await db.select({ total: sql `count(*)` }).from(funnels).where(where);
        const rows = await db.select().from(funnels).where(where).orderBy(desc(funnels.updatedAt)).limit(limit).offset(offset);
        const range = resolveRange();
        const withSummaries = await Promise.all(rows.map(async (row) => {
     ...
     * changes.
     */
    app.get("/orgs/:orgId/sites/:siteId/funnels/:funnelId/analyze", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, funnelId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        const result = await evaluateFunnel(db, site.id, row.steps, range, parsed.data.conversionWindowMinutes ?? row.conversionWindowMinutes, {
    ...
    app.get("/orgs/:orgId/sites/:siteId/funnels/:funnelId/steps/:stepIndex/users", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, funnelId, stepIndex } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const row = await loadFunnelInSite(db, site.id, funnelId);
        ...
        if (!Number.isInteger(index) || index < 0)
            return reply.code(400).send({ error: "invalid_step_index" });
        const parsed = stepUsersQuerySchema.safeParse(request.query);
        if (!parsed.success)
            return reply.code(400).send({ error: "invalid_query", details: parsed.error.flatten() });
        const range = resolveRange(parsed.data.since, parsed.data.until);
        const { users, total } = await getFunnelStepUsers(db, site.id, row.steps, range, parsed.data.conversionWindowMinutes ?? row.conversionWindowMinutes, index, {
            segmentId: parsed.data.segmentId,
