...
function cardJson(row) { const configuration = cardConfigurationSchema.parse(row.configuration); return { ...row, configuration, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) }; }
function dashboardJson(row) { return { ...row, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) }; }
async function invalidReference(db, siteId, cards) {
    ...
    if (funnelIds.length) {
        const found = await db.select({ id: funnels.id }).from(funnels).where(and(eq(funnels.siteId, siteId), inArray(funnels.id, funnelIds)));
        if (new Set(found.map((r) => r.id)).size !== new Set(funnelIds).size)
            return "funnel_not_found";
    }
    if (segmentIds.length) {
        const found = await db.select({ id: segments.id }).from(segments).where(and(eq(segments.siteId, siteId), inArray(segments.id, segmentIds)));
        if (new Set(found.map((r) => r.id)).size !== new Set(segmentIds).size)
            return "segment_not_found";
    }
    if (experienceIds.length) {
        const found = await db.select({ id: experiences.id }).from(experiences).where(and(eq(experiences.siteId, siteId), inArray(experiences.id, experienceIds)));
        if (new Set(found.map(row => row.id)).size !== new Set(experienceIds).size)
            return "experience_not_found";
    }
    return null;
}
export function registerDashboardRoutes(app, db) {
    app.get("/orgs/:orgId/sites/:siteId/dashboards", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId } = request.params;
        const site = await siteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const rows = await db.select({ dashboard: dashboards, cardCount: sql `count(${dashboardCards.id})` }).from(dashboards).leftJoin(dashboardCards, eq(dashboardCards.dashboardId, dashboards.id)).where(eq(dashboards.siteId, siteId)).groupBy(dashboards.id).orderBy(desc(dashboards.updatedAt));
    ...
    app.post("/orgs/:orgId/sites/:siteId/dashboards", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        ...
        const referenceError = await invalidReference(db, siteId, parsed.data.cards);
        if (referenceError)
    ...
    app.get("/orgs/:orgId/sites/:siteId/dashboards/:dashboardId", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, dashboardId } = request.params;
        const site = await siteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const row = await dashboardInSite(db, dashboardId, siteId);
    ...
    app.patch("/orgs/:orgId/sites/:siteId/dashboards/:dashboardId", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        ...
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const existing = await dashboardInSite(db, dashboardId, siteId);
        if (!existing)
            return reply.code(404).send({ error: "dashboard_not_found" });
        ...
        const oldCards = await db.select().from(dashboardCards).where(eq(dashboardCards.dashboardId, dashboardId));
        const oldById = new Map(oldCards.map((c) => [c.id, c]));
        const supplied = parsed.data.cards?.map((c) => c.id).filter((id) => Boolean(id)) ?? [];
