...
import { describeElementIdentity, hasStableIdentity } from "../lib/behavior/elementIdentity.js";
 ...
 */
function tokenForBehavioralEventRow(row) {
...
function labelForBehavioralEventRow(row) {
    const element = row.element ?? undefined;
    return element?.label;
}
export function registerPatternObserverRoutes(app, db) {
    /**
     ...
     */
    app.post("/orgs/:orgId/sites/:siteId/analysis/patterns/observe", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
    ...
    app.get("/orgs/:orgId/sites/:siteId/analysis/patterns/candidates", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
     ...
     */
    app.get("/orgs/:orgId/sites/:siteId/analysis/patterns/candidates/:candidateId", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, candidateId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        for (const link of links) {
            ...
            if (!episode)
                continue; // defensive - a dangling link shouldn't 500 the request
...
//# sourceMappingURL=pattern-observer.js.map