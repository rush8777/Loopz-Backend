...
import { z } from "zod";
import { auditLogs, cuid, memberships, organizationInvitations, organizations, refreshTokens, users, } from "../db/schema.js";
import { env } from "../config.js";
import { authenticate } from "../middleware/authenticate.js";
import { requireOrgRole } from "../middleware/requireOrgRole.js";
import { generateRefreshToken, hashPassword, signAccessToken } from "../lib/auth.js";
import { generateInvitationToken, getInvitationStatus, hashInvitationToken, invitationUrl, normalizeEmail, } from "../lib/invitations.js";
...
function invalidInvitation(reply) {
...
function issueTokenPair(userId, email) {
    const accessToken = signAccessToken({ sub: userId, email }, env.JWT_SECRET);
    const refresh = generateRefreshToken();
    return { accessToken, refresh };
}
export function registerInvitationRoutes(app, db) {
    app.get("/orgs/:orgId/invitations", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const rows = await db
            .select()
            .from(organizationInvitations)
    ...
    app.post("/orgs/:orgId/invitations", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
    ...
    app.delete("/orgs/:orgId/invitations/:invitationId", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
    ...
    app.post("/orgs/:orgId/invitations/:invitationId/rotate", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const { invitationId } = request.params;
        const orgId = request.membership.orgId;
        const [invitation] = await db
            .select()
    ...
    app.get("/auth/invitations/:token", async (request, reply) => {
        ...
        if (!invitation || getInvitationStatus(invitation) !== "pending")
            return invalidInvitation(reply);
    ...
    app.post("/auth/invitations/:token/accept", { preHandler: authenticate }, async (request, reply) => {
        const { token } = request.params;
        const [initial] = await findInvitationByToken(db, token);
        if (!initial || getInvitationStatus(initial) !== "pending")
            return invalidInvitation(reply);
    ...
    app.post("/auth/invitations/:token/signup", async (request, reply) => {
        ...
        const [existingUser] = await db
            ...
            .limit(1);
        if (existingUser)
            return reply.code(409).send({ error: "account_exists_sign_in" });
        ...
        const result = db.transaction((tx) => {
            ...
            if (existing)
                return { error: "account_exists_sign_in" };
            const now = new Date();
            const user = tx
                .insert(users)
