...
export function registerPublicEventsRoutes(app, db) {
    app.post("/public/sites/:siteId/events", async (request, reply) => {
        ...
        const behavioralEvents = events.filter((e) => e.type !== "identify" && e.type !== "session_start");
        ...
        for (const identifyEvent of identifyEvents) {
            if (!identifyEvent.externalUserId)
                continue; // malformed - identify() with no userId, nothing to resolve
        ...
        for (const sessionStartEvent of sessionStartEvents) {
            if (!sessionStartEvent.anonymousId)
                continue; // malformed - can't attribute this session's environment to anyone
