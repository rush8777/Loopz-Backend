...
async function ownSite(db, siteId, orgId) {
...
function matched(rows, page) {
    ...
    return rows.filter((row) => {
        ...
        return Boolean(path && matchesRules(path, page.rules));
    });
}
function ranged(rows, from, to) {
...
function currentSnapshot(rows, page) {
    return rows.find((row) => matchesRules(row.pagePath, page.rules));
}
function metrics(rows, allRows) {
    const visits = rows.filter((r) => r.type === "page_view"), clicks = rows.filter((r) => r.type === "click");
    const groups = new Map();
    for (const row of rows)
        if (row.pageViewId)
            groups.set(row.pageViewId, [...(groups.get(row.pageViewId) ?? []), row.timestamp.getTime()]);
    const durations = [...groups.values()].map((times) => Math.max(...times) - Math.min(...times));
    const last = new Map();
    for (const row of allRows.filter((r) => r.type === "page_view"))
        if (!last.get(row.sessionId) || last.get(row.sessionId).timestamp < row.timestamp)
            last.set(row.sessionId, row);
...
function topClicks(rows) {
    const clicks = rows.filter((r) => r.type === "click"), groups = new Map();
    for (const click of clicks)
        if (click.selector) {
            const item = groups.get(click.selector);
            if (item)
                item.count++;
            else
                groups.set(click.selector, { selector: click.selector, label: click.elementLabel, count: 1 });
        }
...
function reach(rows) {
    const sessions = new Set(rows.filter((r) => r.type === "page_view").map((r) => r.sessionId)), deepest = new Map();
    for (const row of rows.filter((r) => r.type === "scroll" && r.scrollPercent != null)) {
        sessions.add(row.sessionId);
        deepest.set(row.sessionId, Math.max(deepest.get(row.sessionId) ?? 0, row.scrollPercent));
    }
...
export function registerHeatmapRoutes(app, db) {
    app.get("/orgs/:orgId/sites/:siteId/heatmaps", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId } = request.params, site = await ownSite(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const query = dateSchema.safeParse(request.query);
        if (!query.success)
    ...
    app.get("/orgs/:orgId/sites/:siteId/pages/:pageId/heatmap/states", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        ...
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const [page] = await db.select().from(pageDefinitions).where(and(eq(pageDefinitions.id, pageId), eq(pageDefinitions.siteId, site.id))).limit(1);
        if (!page)
    ...
    app.post("/orgs/:orgId/sites/:siteId/pages/:pageId/heatmap/states", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        ...
        if (!page)
            return reply.code(404).send({ error: "page_not_found" });
        const body = z.object({ name: z.string().min(1).max(120), selector: z.string().min(1).max(500) }).safeParse(request.body);
        if (!body.success)
            return reply.code(400).send({ error: "invalid_body" });
        const [state] = await db.insert(pageHeatmapStates).values({ siteId: site.id, pageDefinitionId: page.id, ...body.data }).returning();
        return reply.code(201).send({ id: state.id, name: state.name, selector: state.selector });
    });
    app.get("/orgs/:orgId/sites/:siteId/pages/:pageId/heatmap", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        ...
        if (!page.heatmapEnabled)
            return reply.code(409).send({ error: "heatmap_disabled" });
        const query = dateSchema.extend({ stateId: z.string().default("default"), device: deviceSchema.default("desktop"), layer: layerSchema.default("click") }).safeParse(request.query);
        if (!query.success)
            return reply.code(400).send({ error: "invalid_query" });
        if (query.data.stateId !== "default") {
            const [state] = await db.select().from(pageHeatmapStates).where(and(eq(pageHeatmapStates.id, query.data.stateId), eq(pageHeatmapStates.pageDefinitionId, page.id), eq(pageHeatmapStates.siteId, site.id))).limit(1);
            if (!state)
                return reply.code(404).send({ error: "state_not_found" });
        }
        ...
        const layerRows = pageRows.filter((r) => r.type === query.data.layer), points = new Map();
        for (const row of layerRows) {
            const x = row.documentX ?? row.x ?? undefined, y = row.documentY ?? row.y ?? undefined, key = `${x}:${y}`, weight = row.type === "rage_click" ? row.rageClickCount ?? 1 : 1, item = points.get(key);
            if (item)
                item.count += weight;
            else
                points.set(key, { ...(x != null && { x }), ...(y != null && { y }), count: weight });
        }
    ...
    app.post("/orgs/:orgId/sites/:siteId/pages/:pageId/heatmap/capture-request", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        ...
        if (!body.success)
            return reply.code(400).send({ error: "invalid_body" });
        const [page] = await db.select().from(pageDefinitions).where(and(eq(pageDefinitions.id, pageId), eq(pageDefinitions.siteId, site.id))).limit(1);
        if (!page || !page.heatmapEnabled)
            return reply.code(409).send({ error: "heatmap_disabled" });
        const stateId = body.data.stateId === "default" ? null : body.data.stateId;
        if (stateId) {
            const [state] = await db.select().from(pageHeatmapStates).where(and(eq(pageHeatmapStates.id, stateId), eq(pageHeatmapStates.siteId, site.id), eq(pageHeatmapStates.pageDefinitionId, page.id))).limit(1);
            if (!state)
                return reply.code(404).send({ error: "state_not_found" });
        }
        const siteOrigin = origin(site.domain), path = latestPath(await db.select().from(sessionEvents).where(eq(sessionEvents.siteId, site.id)), page);
        let target = null;
        try {
            target = body.data.targetUrl ? new URL(body.data.targetUrl) : siteOrigin && path ? new URL(path, siteOrigin) : null;
        }
    ...
    app.get("/orgs/:orgId/sites/:siteId/pages/:pageId/heatmap/capture-request/:requestId", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, pageId, requestId } = request.params, site = await ownSite(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        const [capture] = await db.select().from(heatmapCaptureRequests).where(and(eq(heatmapCaptureRequests.id, requestId), eq(heatmapCaptureRequests.siteId, site.id), eq(heatmapCaptureRequests.pageDefinitionId, pageId))).limit(1);
        if (!capture)
    ...
    app.get("/public/sites/:siteId/heatmap-reference", async (request, reply) => {
        const { siteId } = request.params, query = z.object({ path: z.string().min(1).max(2000), device: deviceSchema }).safeParse(request.query);
        if (!query.success)
            return reply.code(400).send({ error: "invalid_query" });
        ...
        const snapshots = await db.select().from(heatmapReferenceSnapshots).where(and(eq(heatmapReferenceSnapshots.siteId, site.id), eq(heatmapReferenceSnapshots.pageDefinitionId, page.id), eq(heatmapReferenceSnapshots.deviceClass, query.data.device), isNull(heatmapReferenceSnapshots.pageStateId)));
        if (currentSnapshot(snapshots, page))
            return reply.send({ capture: null });
        const requests = await db.select().from(heatmapCaptureRequests).where(and(eq(heatmapCaptureRequests.siteId, site.id), eq(heatmapCaptureRequests.pageDefinitionId, page.id), eq(heatmapCaptureRequests.deviceClass, query.data.device), isNull(heatmapCaptureRequests.pageStateId)));
        if (requests.some((r) => !r.usedAt && r.expiresAt > new Date()))
            return reply.send({ capture: null });
        const [capture] = await db.insert(heatmapCaptureRequests).values({ token: nanoid(32), siteId: site.id, pageDefinitionId: page.id, pageStateId: null, deviceClass: query.data.device, expiresAt: new Date(Date.now() + 5 * 60_000) }).returning();
        return reply.send({ capture: { token: capture.token } });
    });
    app.get("/public/sites/:siteId/heatmap-captures/:token", async (request, reply) => {
        ...
        if (!capture || capture.usedAt || capture.expiresAt < new Date())
            return reply.code(404).send({ error: "capture_not_found" });
        const [page] = await db.select().from(pageDefinitions).where(and(eq(pageDefinitions.id, capture.pageDefinitionId), eq(pageDefinitions.siteId, site.id))).limit(1);
        if (!page)
            return reply.code(404).send({ error: "capture_not_found" });
    ...
    app.post("/public/sites/:siteId/heatmap-snapshots/:token", { bodyLimit: 8_500_000 }, async (request, reply) => {
        ...
        if (!body.success || body.data.deviceClass !== capture.deviceClass)
            return reply.code(400).send({ error: "invalid_snapshot" });
        const [page] = await db.select().from(pageDefinitions).where(and(eq(pageDefinitions.id, capture.pageDefinitionId), eq(pageDefinitions.siteId, site.id))).limit(1);
        if (!page || !matchesRules(body.data.pagePath, page.rules))
            return reply.code(400).send({ error: "page_path_mismatch" });
        await db.insert(heatmapReferenceSnapshots).values({ siteId: site.id, pageDefinitionId: page.id, pageStateId: capture.pageStateId, ...body.data, capturedAt: new Date() });
        await db.update(heatmapCaptureRequests).set({ usedAt: new Date() }).where(and(eq(heatmapCaptureRequests.id, capture.id), eq(heatmapCaptureRequests.siteId, site.id)));
