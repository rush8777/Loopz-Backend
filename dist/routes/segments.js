...
export function registerSegmentRoutes(app, db) {
    /** Segments list - searched server-side by name (task brief section 16), not loaded in full then filtered client-side. */
    app.get("/orgs/:orgId/sites/:siteId/segments", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        ...
        if (search)
            conditions.push(like(segments.name, `%${search}%`));
        const where = and(...conditions);
        const [{ total }] = await db.select({ total: sql `count(*)` }).from(segments).where(where);
        ...
        const segmentsWithCounts = await Promise.all(rows.map(async (row) => serializeSegment(row, await getSegmentAudienceCount(db, site.id, row.definition))));
    ...
    app.post("/orgs/:orgId/sites/:siteId/segments", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const { siteId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        const [row] = await db
            ...
            .returning();
        const audienceCount = await getSegmentAudienceCount(db, site.id, row.definition);
     ...
     */
    app.post("/orgs/:orgId/sites/:siteId/segments/preview", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
    ...
    app.get("/orgs/:orgId/sites/:siteId/segments/:segmentId", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, segmentId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
    ...
    app.get("/orgs/:orgId/sites/:siteId/segments/:segmentId/members", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId, segmentId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        const { members, total } = await getSegmentMembers(db, site.id, row.definition, { limit, offset });
        return reply.send({ members, total, limit, offset });
    });
    app.patch("/orgs/:orgId/sites/:siteId/segments/:segmentId", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const { siteId, segmentId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        const [updated] = await db
            ...
            .returning();
        const audienceCount = await getSegmentAudienceCount(db, site.id, updated.definition);
     ...
     */
    app.delete("/orgs/:orgId/sites/:siteId/segments/:segmentId", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const { siteId, segmentId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
