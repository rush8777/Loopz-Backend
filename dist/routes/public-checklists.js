...
import { checklistProgress, completeChecklistItemFromClick, currentChecklistDefinition, getOrCreateChecklistState, refreshChecklist } from "../lib/experiences/checklistProgress.js";
...
const launchSchema = z.object({ ...identitySchema, source: z.enum(["api", "checklist"]), checklistExperienceId: z.string().min(1).max(64).optional(), itemId: z.string().min(1).max(64).optional() }).strict().superRefine((value, ctx) => { if (value.source === "checklist" && (!value.checklistExperienceId || !value.itemId))
    ctx.addIssue({ code: "custom", path: ["checklistExperienceId"], message: "Checklist launch context is required" }); });
async function trackedIdentity(db, siteId, anonymousId, externalId) {
    if (!externalId)
        return { anonymousId, trackedUserId: null };
...
function checkedUrl(urlValue, siteDomain) {
    try {
        const url = new URL(urlValue);
        if (siteDomain) {
            const origin = new URL(/^https?:\/\//i.test(siteDomain) ? siteDomain : `https://${siteDomain}`).origin;
...
async function audienceMatches(db, siteId, identityKey, audience) {
    if (audience.type === "all")
        return true;
    const conditions = audience.type === "segment" ? [{ segmentId: audience.segmentId, operator: "matches" }] : audience.conditions;
    const values = await Promise.all(conditions.map(async (condition) => { const [segment] = await db.select().from(segments).where(eq(segments.id, condition.segmentId)).limit(1); if (!segment || segment.siteId !== siteId)
        return false; const member = (await evaluateSegment(db, siteId, segment.definition)).has(identityKey); return condition.operator === "matches" ? member : !member; }));
    return audience.type === "segment_rules" && audience.logic === "any" ? values.some(Boolean) : values.every(Boolean);
}
function available(definition, url) {
    ...
    if (target.schedule?.endsAt && now >= new Date(target.schedule.endsAt))
        return false;
    if (target.allowedOrigins?.length && !target.allowedOrigins.includes(url.origin))
        return false;
    const path = `${url.pathname}${url.search}${url.hash}`;
    return !target.pageRules.length || matchesRules(path, target.pageRules);
}
export async function deliveredChecklist(db, site, input) {
    const url = checkedUrl(input.url, site.domain);
    if (!url)
        return null;
    const identity = await trackedIdentity(db, site.id, input.anonymousId, input.trackedUserId);
    ...
    for (const experience of rows) {
        ...
        if (!version || version.state !== "published")
            continue;
        const parsed = checklistDefinitionSchema.safeParse(version.definition);
        if (!parsed.success || !available(parsed.data, url) || !await audienceMatches(db, site.id, identityKey, parsed.data.targeting.audience))
            continue;
        ...
        if (progress.dismissed || progress.completionAcknowledged)
            continue;
    ...
    const progress = await checklistProgress(db, state, candidate.definition);
    const { targeting: _targeting, ...definition } = candidate.definition;
    return { id: candidate.experience.id, versionId: candidate.version.id, kind: "checklist", priority: candidate.definition.targeting.priority, definition, progress };
}
export function registerPublicChecklistRoutes(app, db) {
    app.post("/public/sites/:siteId/checklists/:experienceId/refresh", async (request, reply) => {
        ...
        if (!site || !checkedUrl(body.data.url, site.domain))
            return reply.code(404).send({ error: "checklist_not_found" });
        const current = await currentChecklistDefinition(db, site.id, experienceId);
        if (!current || current.version.id !== body.data.versionId)
            return reply.code(404).send({ error: "checklist_not_found" });
        const parsed = checklistDefinitionSchema.safeParse(current.definition);
        if (!parsed.success)
            return reply.code(409).send({ error: "invalid_checklist" });
        const identity = await trackedIdentity(db, site.id, body.data.anonymousId, body.data.trackedUserId);
        if (!available(parsed.data, new URL(body.data.url)) || !await audienceMatches(db, site.id, identity.trackedUserId ?? identity.anonymousId, parsed.data.targeting.audience))
            return reply.code(409).send({ error: "checklist_unavailable" });
    ...
    app.post("/public/sites/:siteId/checklists/:experienceId/actions", async (request, reply) => {
        ...
        if (!current || current.version.id !== body.data.versionId)
            return reply.code(404).send({ error: "checklist_not_found" });
        const parsed = checklistDefinitionSchema.safeParse(current.definition);
        if (!parsed.success || !available(parsed.data, url))
            return reply.code(409).send({ error: "checklist_unavailable" });
        const identity = await trackedIdentity(db, site.id, body.data.anonymousId, body.data.trackedUserId);
        if (!await audienceMatches(db, site.id, identity.trackedUserId ?? identity.anonymousId, parsed.data.targeting.audience))
            return reply.code(409).send({ error: "checklist_unavailable" });
        const state = await getOrCreateChecklistState(db, site.id, experienceId, identity, parsed.data.behavior.initialState === "collapsed");
        const now = new Date(body.data.timestamp);
        const before = await checklistProgress(db, state, parsed.data);
        if (body.data.action === "dismiss" && !parsed.data.behavior.dismissible)
            return reply.code(409).send({ error: "dismissal_disabled" });
        if (body.data.action === "completion_acknowledged" && !before.complete)
            return reply.code(409).send({ error: "checklist_incomplete" });
        if (body.data.action === "item_click") {
            ...
            if ("error" in result)
                return reply.code(result.error === "item_locked" ? 409 : 404).send({ error: result.error });
        ...
        if (body.data.action === "shown") {
            const [impression] = await db.insert(experienceImpressions).values({ siteId: site.id, experienceId, versionId: current.version.id, anonymousId: identity.anonymousId, trackedUserId: identity.trackedUserId, sessionId: body.data.sessionId, pageViewId: body.data.pageViewId ?? null, shownAt: now }).returning();
        ...
        if (body.data.action === "dismiss" && impressionId)
            await db.update(experienceImpressions).set({ dismissedAt: now }).where(eq(experienceImpressions.id, impressionId));
    ...
    app.post("/public/sites/:siteId/experiences/:experienceId/launch", async (request, reply) => {
        ...
        if (!experience?.publishedVersionId)
            return reply.code(404).send({ error: "experience_not_found" });
        const [version] = await db.select().from(experienceVersions).where(eq(experienceVersions.id, experience.publishedVersionId)).limit(1);
        const parsed = version && guideDefinitionSchema.safeParse(version.definition);
        if (!version || version.state !== "published" || !parsed || !parsed.success)
            return reply.code(404).send({ error: "experience_not_found" });
        const target = parsed.data.targeting;
        const now = new Date();
        if ((target.schedule?.startsAt && now < new Date(target.schedule.startsAt)) || (target.schedule?.endsAt && now >= new Date(target.schedule.endsAt)) || (target.allowedOrigins?.length && !target.allowedOrigins.includes(url.origin)))
            return reply.code(409).send({ error: "experience_unavailable" });
        if (body.data.source === "checklist") {
            const source = await currentChecklistDefinition(db, site.id, body.data.checklistExperienceId);
            const sourceDefinition = source && checklistDefinitionSchema.safeParse(source.definition);
            if (!source || !sourceDefinition || !sourceDefinition.success || !available(sourceDefinition.data, url))
                return reply.code(409).send({ error: "checklist_unavailable" });
            const itemIndex = sourceDefinition.data.items.findIndex(item => item.id === body.data.itemId && item.action.type === "launch_guide" && item.action.experienceId === experience.id);
            if (itemIndex < 0)
                return reply.code(409).send({ error: "invalid_launch_context" });
            const identity = await trackedIdentity(db, site.id, body.data.anonymousId, body.data.trackedUserId);
            if (!await audienceMatches(db, site.id, identity.trackedUserId ?? identity.anonymousId, sourceDefinition.data.targeting.audience))
                return reply.code(409).send({ error: "checklist_unavailable" });
            const state = await getOrCreateChecklistState(db, site.id, source.experience.id, identity, sourceDefinition.data.behavior.initialState === "collapsed");
            const progress = await checklistProgress(db, state, sourceDefinition.data);
            if (progress.items[itemIndex]?.state === "locked" || progress.dismissed || progress.completionAcknowledged)
                return reply.code(409).send({ error: "checklist_unavailable" });
        }
...
//# sourceMappingURL=public-checklists.js.map