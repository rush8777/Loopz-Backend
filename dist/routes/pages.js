...
import { loadPagePathStats, computeMatchedMetrics } from "../lib/pages/pageAggregation.js";
...
function hasDuplicateRuleIds(rules) {
    const ids = rules.map((r) => r.id);
    return new Set(ids).size !== ids.length;
}
function serializePage(row, metrics) {
    return {
        id: row.id,
        siteId: row.siteId,
        name: row.name,
...
export function registerPageRoutes(app, db) {
     ...
     * codebase.
     */
    app.get("/orgs/:orgId/sites/:siteId/pages/untagged", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        const { siteId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        const ruleSets = pageRows.map((row) => row.rules);
     ...
     */
    app.post("/orgs/:orgId/sites/:siteId/pages/preview", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const { siteId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        const metrics = await computeMatchedMetrics(db, site.id, matched.map((p) => p.pagePath));
        return reply.send({
    ...
    app.get("/orgs/:orgId/sites/:siteId/pages/:pageId", { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] }, async (request, reply) => {
        ...
        const metrics = await computeMatchedMetrics(db, site.id, matchedPaths.map((p) => p.pagePath));
        return reply.send({
    ...
    app.patch("/orgs/:orgId/sites/:siteId/pages/:pageId", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const { siteId, pageId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
        ...
        const [updated] = await db
            ...
            .returning();
        const pathStats = await loadPagePathStats(db, site.id);
        const matchedPaths = filterMatchingPaths(pathStats.map((p) => p.pagePath), updated.rules);
        const metrics = await computeMatchedMetrics(db, site.id, matchedPaths);
        return reply.send(serializePage(updated, metrics));
    });
    app.delete("/orgs/:orgId/sites/:siteId/pages/:pageId", { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] }, async (request, reply) => {
        const { siteId, pageId } = request.params;
        const site = await loadSiteInOrg(db, siteId, request.membership.orgId);
        if (!site)
            return reply.code(404).send({ error: "site_not_found" });
