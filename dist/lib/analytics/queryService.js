...
async function loadFacts(db, siteId, since, until) {
    ...
    return rows.map((r) => ({ ...r, identity: r.trackedUserId ?? r.anonymousId })).filter((r) => Boolean(r.identity));
}
const round = (n) => Math.round(n * 100) / 100;
const meaningful = (f, excluded) => ["page_view", "click", "scroll", "custom"].includes(f.type) && !(f.type === "custom" && f.eventName && excluded.has(f.eventName));
const customNamed = (f, names) => f.type === "custom" && (!names || names.includes(f.eventName ?? ""));
const unique = (facts, key) => new Set(facts.map((f) => f[key])).size;
function applyEventPool(facts, config, excluded, eventName) {
    if (eventName)
        return facts.filter((f) => customNamed(f, [eventName]) && !excluded.has(eventName));
    if (config.events)
        return facts.filter((f) => customNamed(f, config.events.eventNames) && !excluded.has(f.eventName ?? ""));
...
function qualifyAll(facts, config, key) {
    if (config.events?.match !== "all")
        return facts;
    const byEntity = new Map();
    for (const f of facts)
        if (f.eventName) {
            const set = byEntity.get(f[key]) ?? new Set();
            set.add(f.eventName);
            byEntity.set(f[key], set);
        }
...
function metricValue(allFacts, config, excluded, eventName) {
    ...
    if (config.metricId.startsWith("users."))
        return unique(qualifyAll(pool, config, "identity"), "identity");
    if (config.metricId === "sessions.count")
        return unique(qualifyAll(pool, config, "sessionId"), "sessionId");
    ...
    if (config.metricId === "events.occurrences")
        return pool.length;
    if (config.metricId === "events.per_user")
        return unique(pool, "identity") ? round(pool.length / unique(pool, "identity")) : 0;
    if (config.metricId === "events.per_session")
        return unique(pool, "sessionId") ? round(pool.length / unique(pool, "sessionId")) : 0;
    return 0;
}
async function segmentMembers(db, siteId, segmentId) {
    if (!segmentId)
        return undefined;
    const [row] = await db.select().from(segments).where(and(eq(segments.id, segmentId), eq(segments.siteId, siteId))).limit(1);
    if (!row)
        throw new AnalyticsError("segment_not_found", "The selected Segment does not belong to this site.");
    return evaluateSegment(db, siteId, row.definition);
}
async function executeMetric(db, siteId, filters, config) {
    const catalog = METRIC_CATALOG.find((m) => m.id === config.metricId);
    if (config.breakdown && !catalog.breakdowns.includes(config.breakdown.dimension))
        throw new AnalyticsError("unsupported_combination", "This metric does not support the selected breakdown.");
    ...
    if (members)
        facts = facts.filter((f) => members.has(f.identity));
    ...
    if (config.metricId === "users.stickiness") {
        ...
        if (config.mode === "trend")
            return { kind: "timeseries", series, buckets };
        const values = daily.map((d) => d.value);
        return { kind: "scalar", values: [{ seriesKey: "value", value: values.length ? round(values.reduce((a, v) => a + v, 0) / values.length) : 0, recentValue: values.at(-1) ?? 0, previousValue: null, deltaPercent: null }] };
    }
    if (config.mode === "trend") {
    ...
    if (config.mode === "single") {
        ...
        return { kind: "scalar", values: series.map((s) => { const value = metricValue(rangeFacts, config, exclusions, s.key === "value" ? undefined : s.key); const recentValue = lastBucket ? metricValue(rangeFacts.filter((f) => f.at >= lastBucket.start && f.at < lastBucket.end), config, exclusions, s.key === "value" ? undefined : s.key) : 0; const previousValue = metricValue(previousScoped, config, exclusions, s.key === "value" ? undefined : s.key); return { seriesKey: s.key, value, recentValue, previousValue, deltaPercent: previousValue ? round((value - previousValue) / previousValue * 100) : null }; }) };
    }
    return executeBreakdown(db, siteId, rangeFacts, config, exclusions);
}
async function executeBreakdown(db, siteId, facts, config, exclusions) {
    const breakdown = config.breakdown;
    const groups = new Map();
    const add = (key, label, fact) => { const g = groups.get(key) ?? { label, facts: [] }; g.facts.push(fact); groups.set(key, g); };
    if (breakdown.dimension === "segment") {
        for (const id of breakdown.segmentIds) {
            ...
            if (!row)
                throw new AnalyticsError("segment_not_found", "A breakdown Segment does not belong to this site.");
        ...
        for (const fact of facts) {
            const c = contextMap.get(fact.sessionId);
            let values = [];
            if (breakdown.dimension === "event")
                values = fact.type === "custom" && fact.eventName ? [{ key: fact.eventName, label: fact.eventName }] : [];
            else if (breakdown.dimension === "identity")
                values = [{ key: fact.trackedUserId ? "identified" : "anonymous", label: fact.trackedUserId ? "Identified" : "Anonymous" }];
            else if (breakdown.dimension === "device")
                values = [{ key: c?.deviceType ?? "unknown", label: c?.deviceType ?? "Unknown" }];
            else if (breakdown.dimension === "browser")
                values = [{ key: c?.browserName ?? "unknown", label: c?.browserName ?? "Unknown" }];
            else if (breakdown.dimension === "os")
                values = [{ key: c?.osName ?? "unknown", label: c?.osName ?? "Unknown" }];
            else if (breakdown.dimension === "language")
                values = [{ key: c?.language ?? "unknown", label: c?.language ?? "Unknown" }];
            else if (breakdown.dimension === "referrer")
                values = [{ key: c?.referrer ?? "direct", label: c?.referrer ?? "Direct / unknown" }];
            else if (breakdown.dimension === "user_property") {
                const v = fact.trackedUserId ? propertyMap.get(fact.trackedUserId) : undefined;
                values = [{ key: v ?? "unknown", label: v ?? "Unknown" }];
            }
            else if (["page", "page_area", "page_type"].includes(breakdown.dimension) && fact.pagePath)
                values = pages.filter((p) => matchesRules(fact.pagePath, p.rules)).map((p) => breakdown.dimension === "page" ? { key: p.id, label: p.name } : breakdown.dimension === "page_area" ? { key: p.area ?? "unknown", label: p.area ?? "Unknown" } : { key: p.pageType ?? "unknown", label: p.pageType ?? "Unknown" });
            for (const value of new Map(values.map((v) => [v.key, v])).values())
                add(value.key, value.label, fact);
        }
    }
...
function metadata(filters, query, result) {
...
export async function executeAnalyticsQuery(db, siteId, filters, query) {
    let result;
    if (query.kind === "metric")
        result = await executeMetric(db, siteId, filters, query);
    else if (query.kind === "experience") {
        result = await experienceMetric(db, siteId, query.experienceId, query.metric, { since: new Date(filters.since), until: new Date(filters.until) });
        if (!result)
            throw new AnalyticsError("experience_not_found", "The Experience does not belong to this site.", 404);
    }
    else if (query.kind === "retention")
        result = await computeRetention(db, siteId, filters, query);
    else {
        const [row] = await db.select().from(funnels).where(and(eq(funnels.id, query.funnelId), eq(funnels.siteId, siteId))).limit(1);
        if (!row)
            throw new AnalyticsError("funnel_not_found", "The saved Funnel does not belong to this site.");
        result = { kind: "funnel", ...(await evaluateFunnel(db, siteId, row.steps, { since: new Date(filters.since), until: new Date(filters.until) }, row.conversionWindowMinutes, { segmentId: filters.segmentId })) };
    }
    return { metadata: metadata(filters, query, result), result };
}
export async function executeDrilldown(db, siteId, filters, query, selection) {
    if (query.kind === "experience")
        throw new AnalyticsError("drilldown_not_supported", "Experience cards do not expose raw event drilldown.");
    if (query.kind === "funnel") {
        const [row] = await db.select().from(funnels).where(and(eq(funnels.id, query.funnelId), eq(funnels.siteId, siteId))).limit(1);
        if (!row)
            throw new AnalyticsError("funnel_not_found", "The saved Funnel does not belong to this site.");
        const page = await getFunnelStepUsers(db, siteId, row.steps, { since: new Date(filters.since), until: new Date(filters.until) }, row.conversionWindowMinutes, selection.stepIndex ?? 0, { segmentId: filters.segmentId, offset: selection.offset, limit: selection.limit });
    ...
    let facts = await loadFacts(db, siteId, since, until);
    const members = await segmentMembers(db, siteId, filters.segmentId);
    if (members)
        facts = facts.filter((f) => members.has(f.identity));
    if (query.kind === "retention" || query.metricId.startsWith("users.")) {
        ...
        const config = query.kind === "metric" ? query : { schemaVersion: 1, kind: "metric", metricId: "users.unique", mode: "single", visualization: "total" };
