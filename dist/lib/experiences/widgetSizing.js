...
export function defaultWidgetSize(widgetType) {
...
function sizeIsValid(widgetType, size) {
    const { width, height } = size;
    const constraint = WIDGET_SIZE_CONSTRAINTS[widgetType];
    if (widgetType === "banner")
        return width.mode === "full" && height.mode === "auto";
    if (width.mode !== "fixed" && !(width.mode === "full" && constraint.width.allowFull))
        return false;
    if (width.mode === "fixed" && (width.value === undefined || width.value < constraint.width.min || width.value > constraint.width.max))
        return false;
    if (height.mode === "fixed" && (!constraint.height.allowFixed || height.value === undefined || height.value < constraint.height.min || height.value > constraint.height.max))
        return false;
    if (height.mode === "viewport" && !constraint.height.allowViewport)
        return false;
    return true;
}
export function widgetSizeIsValid(widgetType, definition) {
    if ("steps" in definition || "items" in definition)
        return true;
    const sizes = [definition.design.size, ...(widgetType === "survey" ? definition.survey?.steps.map(step => step.size) ?? [] : [])].filter((size) => Boolean(size));
    return sizes.every(size => sizeIsValid(widgetType, size));
}
//# sourceMappingURL=widgetSizing.js.map