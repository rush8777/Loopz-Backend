...
import { createClickEvent, createCustomEvent, createDwellEvent, createElementApproachEvent, createElementLeaveEvent, createHesitationEvent, createHoverIntentEvent, createPageEnterEvent, createRepeatedAttentionEvent, createReversalEvent, createScrollEvent, } from "./behavioralEvent.js";
...
// ---------------------------------------------------------------------------
function computeRepeatedAttentionEvents(visitsBySelector, labelBySelector, config) {
    const events = [];
    for (const [selector, rawTimestamps] of visitsBySelector) {
        ...
        for (let i = 1; i <= timestamps.length; i++) {
            ...
            if (cluster.length >= 2) {
                ...
                events.push(createRepeatedAttentionEvent(cluster[cluster.length - 1], element, cluster.length, {
    ...
    function handleAnchorEvent(event) {
        ...
        if (currentAnchor) {
            ...
            if (newSelector && newSelector !== currentAnchor.selector) {
                output.push(...classifyApproachRun(pendingRun, { selector: newSelector, label: newLabel, role: newRole, position: newPosition }, event.timestamp, config, clickableSelectors));
            }
        }
        ...
        if (newSelector && (newLabel || newRole))
            labelBySelector.set(newSelector, { label: newLabel, role: newRole });
    ...
    for (const event of sorted) {
        switch (event.type) {
            ...
            case "custom": {
                ...
                if (event.name)
                    output.push(createCustomEvent(event.timestamp, event.name, event.properties));
                break;
            }
            default:
                break;
