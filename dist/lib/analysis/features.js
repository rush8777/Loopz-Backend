function tokenFor(event) {
    ...
    if (!event.element?.selector)
        return event.type; // e.g. plain "scroll" - no target element to qualify it with
    return `${event.type}:${event.element.selector}`;
}
function matchesGoal(event, goal) {
    if (event.type !== goal.type)
        return false;
    if (goal.selector != null && event.element?.selector !== goal.selector)
        return false;
    return true;
}
/**
 ...
 */
export function extractSessionFeatures(sessionId, events, goal) {
    ...
    for (const event of sorted) {
        ...
        if (event.element?.selector)
            targets.add(event.element.selector);
        switch (event.type) {
            case "click":
                clickCount += 1;
                break;
        ...
        if (goal && matchesGoal(event, goal))
            converted = true;
    }
    const sessionDurationMs = sorted.length > 0 ? sorted[sorted.length - 1].timestamp - sorted[0].timestamp : 0;
    return {
        sessionId,
