 ...
 */
export function standardize(vectors) {
    ...
    for (const v of vectors) {
        for (let d = 0; d < dims; d++)
            stdevs[d] += (v[d] - means[d]) ** 2;
    }
    for (let d = 0; d < dims; d++)
        stdevs[d] = Math.sqrt(stdevs[d] / n) || 1; // avoid div-by-zero for constant dimensions
...
function initCentroids(vectors, k, rng) {
