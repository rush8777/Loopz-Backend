...
describe("visual experiences", () => {
  ...
  it("persists and publishes safe builder data while rejecting unsafe markup and CSS", async () => {
    ...
    definition.builder.html = '<section class="movecues-widget"><script>alert(1)</script></section>'; const invalidMarkup = await ctx.app.inject({ method: "PATCH", url: `/orgs/${owner.org.id}/sites/${site.id}/experiences/${created.id}`, headers: authorization, payload: { definition } }); expect(invalidMarkup.statusCode).toBe(400); expect(invalidMarkup.json().details.issues.length).toBeGreaterThan(0);
    const afterInvalid = await ctx.app.inject({ method: "GET", url: `/orgs/${owner.org.id}/sites/${site.id}/experiences/${created.id}`, headers: authorization }); expect(afterInvalid.json().draftVersion.definition.builder.html).toContain("Unpublished draft builder");
    definition.builder.html = '<section class="movecues-widget"><iframe src="https://example.test"></iframe></section>'; expect((await ctx.app.inject({ method: "PATCH", url: `/orgs/${owner.org.id}/sites/${site.id}/experiences/${created.id}`, headers: authorization, payload: { definition } })).statusCode).toBe(400);
    definition.builder.html = '<section class="movecues-widget"><img src="data:image/svg+xml;base64,unsafe"></section>'; expect((await ctx.app.inject({ method: "PATCH", url: `/orgs/${owner.org.id}/sites/${site.id}/experiences/${created.id}`, headers: authorization, payload: { definition } })).statusCode).toBe(400);
  ...
  it("creates and publishes a validated multi-step survey and persists submitted or abandoned structured responses", async () => {
    ...
    definition.survey.steps[0].builder.html = definition.survey.steps[0].builder.html.replace('class="movecues-survey-footer"', 'class="movecues-survey-footer" data-movecues-survey-controls="builder"');
