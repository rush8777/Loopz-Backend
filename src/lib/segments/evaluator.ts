...
async function resolveMatchedPaths(db: Db, siteId: string, pageId: string): Promise<string[] | null> {
...
export async function getSegmentMembers(
  ...
  if (page.length === 0) return { members: [], total };
  ...
  const members: SegmentMember[] = page.map((id) => {
