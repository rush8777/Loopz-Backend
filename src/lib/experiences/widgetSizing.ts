...
export function widgetSizeIsValid(widgetType: WidgetType, definition: ExperienceDefinition): boolean {
  if ("steps" in definition || !definition.design.size) return true;
  const { width, height } = definition.design.size; const constraint = WIDGET_SIZE_CONSTRAINTS[widgetType];
