...
export const checklistDefinitionSchema = z.object({
  ...
  definition.items.forEach((item, index) => { if (ids.has(item.id)) ctx.addIssue({ code: "custom", path: ["items", index, "id"], message: "Checklist item IDs must be unique" }); ids.add(item.id); if (item.action.type === "none" && item.completion.type !== "item_clicked") ctx.addIssue({ code: "custom", path: ["items", index, "action"], message: "Items without an action must complete when clicked" }); });
  const rootCount = Array.from(definition.builder.html.matchAll(/data-movecues-checklist-role\s*=\s*["']root["']/gi)).length;
  if (rootCount !== 1) ctx.addIssue({ code: "custom", path: ["builder", "html"], message: "Checklist builder must contain exactly one root marker" });
  for (const role of ["title", "items", "progress", "launcher-label", "remaining-count", "completion-title", "completion-description", "completion-acknowledge"]) {
    if (!new RegExp(`data-movecues-checklist-role\\s*=\\s*["']${role}["']`, "i").test(definition.builder.html)) ctx.addIssue({ code: "custom", path: ["builder", "html"], message: `Checklist builder is missing ${role}` });
  }
  const markers = Array.from(definition.builder.html.matchAll(/data-movecues-checklist-item-id\s*=\s*["']([^"']+)["']/gi)).map(match => match[1]);
  definition.items.forEach(item => { if (markers.filter(id => id === item.id).length !== 1) ctx.addIssue({ code: "custom", path: ["builder", "html"], message: `Checklist item ${item.id} must appear exactly once` }); });
  markers.forEach(id => { if (!ids.has(id)) ctx.addIssue({ code: "custom", path: ["builder", "html"], message: `Unknown Checklist item marker ${id}` }); });
});
...
export const createExperienceSchema = z.object({
  ...
  if (value.kind !== "widget" && value.widgetType) ctx.addIssue({ code: "custom", path: ["widgetType"], message: `${value.kind}s do not have a widgetType` });
  if (value.kind !== "checklist" && !value.buildPageId && !value.buildUrl) ctx.addIssue({ code: "custom", path: ["buildUrl"], message: "a build page or URL is required" });
  if (value.kind === "checklist" && value.template === "blank") ctx.addIssue({ code: "custom", path: ["template"], message: "choose a Checklist preset" });
  if (value.kind !== "checklist" && value.template !== "blank") ctx.addIssue({ code: "custom", path: ["template"], message: "presets are only supported for Checklists" });
});
...
export const impressionSchema = z.object({
  ...
  if (value.launchContext?.launchSource === "checklist" && (!value.launchContext.sourceExperienceId || !value.launchContext.sourceItemId)) ctx.addIssue({ code: "custom", path: ["launchContext"], message: "Checklist launch attribution is incomplete" });
});
...
export function definitionSchemaFor(kind: "guide" | "widget" | "checklist", widgetType?: string | null) {
  if (kind === "guide") return guideDefinitionSchema;
  if (kind === "checklist") return checklistDefinitionSchema;
