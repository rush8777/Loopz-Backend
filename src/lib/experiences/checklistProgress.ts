...
import type { ChecklistExperienceDefinition, ChecklistItem } from "./types.js";

export type ChecklistCompletionSource = "segment" | "guide_completed" | "item_clicked";

export interface ChecklistIdentity {
  anonymousId: string;
  trackedUserId: string | null;
}

export async function getOrCreateChecklistState(db: Db, siteId: string, experienceId: string, identity: ChecklistIdentity, initialCollapsed: boolean) {
...
export async function checklistProgress(db: Db, state: typeof checklistStates.$inferSelect, definition: ChecklistExperienceDefinition) {
  ...
  const itemStates = definition.items.map((item, index) => ({ id: item.id, state: completedIds.has(item.id) ? "completed" as const : definition.behavior.order === "sequential" && definition.items.slice(0, index).some(previous => !completedIds.has(previous.id)) ? "locked" as const : "available" as const }));
...
async function conditionMatches(db: Db, siteId: string, identity: ChecklistIdentity, item: ChecklistItem): Promise<ChecklistCompletionSource | null> {
  if (item.completion.type === "item_clicked") return null;
  ...
  if (item.completion.type === "segment") {
    ...
    if (!segment || segment.siteId !== siteId) return null;
    try { return (await evaluateSegment(db, siteId, segment.definition as SegmentDefinition)).has(identityKey) ? "segment" : null; }
    catch { return null; }
  }
...
async function insertCompletion(db: Db, stateId: string, versionId: string, item: ChecklistItem, itemIndex: number, source: ChecklistCompletionSource, context: { siteId: string; experienceId: string; identity: ChecklistIdentity; sessionId?: string; pageViewId?: string }) {
  const now = new Date();
...
export async function refreshChecklist(db: Db, input: { siteId: string; experienceId: string; versionId: string; definition: ChecklistExperienceDefinition; identity: ChecklistIdentity; sessionId?: string; pageViewId?: string }) {
  ...
  if (progress.dismissed || (progress.completionAcknowledged && progress.complete)) return { ...progress, newlyCompletedIds: [] as string[] };
  ...
  const distinct = new Map<string, Promise<ChecklistCompletionSource | null>>();
  for (const item of candidates) {
    if (item.completion.type === "item_clicked") continue;
    const key = `${item.completion.type}:${item.completion.type === "segment" ? item.completion.segmentId : item.completion.experienceId}`;
    if (!distinct.has(key)) distinct.set(key, conditionMatches(db, input.siteId, input.identity, item));
  }
  ...
  for (const item of candidates) {
    ...
    if (source && await insertCompletion(db, state.id, input.versionId, item, input.definition.items.indexOf(item), source, input)) newlyCompletedIds.push(item.id);
  }
...
export async function completeChecklistItemFromClick(db: Db, input: { siteId: string; experienceId: string; versionId: string; definition: ChecklistExperienceDefinition; identity: ChecklistIdentity; itemId: string; sessionId?: string; pageViewId?: string }) {
  const state = await getOrCreateChecklistState(db, input.siteId, input.experienceId, input.identity, input.definition.behavior.initialState === "collapsed");
  ...
  if (itemIndex < 0) return { error: "item_not_found" as const };
  if (progress.items[itemIndex].state === "locked") return { error: "item_locked" as const };
  const item = input.definition.items[itemIndex]; const now = new Date();
  if (!state.startedAt) await db.update(checklistStates).set({ startedAt: now, isCollapsed: true, updatedAt: now }).where(eq(checklistStates.id, state.id));
...
export async function claimChecklistProgress(db: Db, siteId: string, anonymousId: string, trackedUserId: string) {
  ...
  for (const anonymousState of anonymousStates) {
    ...
    for (const completion of completions) await db.insert(checklistItemCompletions).values({ checklistStateId: identified.id, itemId: completion.itemId, completedAt: completion.completedAt, completionSource: completion.completionSource, completedVersionId: completion.completedVersionId }).onConflictDoNothing();
...
function latest(a: Date | null, b: Date | null) { return a && b ? new Date(Math.max(a.getTime(), b.getTime())) : a ?? b; }
function earliest(a: Date | null, b: Date | null) { return a && b ? new Date(Math.min(a.getTime(), b.getTime())) : a ?? b; }

async function recomputeClaimedChecklistCompletion(db: Db, siteId: string, experienceId: string, stateId: string) {
  const current = await currentChecklistDefinition(db, siteId, experienceId);
  if (!current || !current.definition || !Array.isArray(current.definition.items)) return;
...
export async function currentChecklistDefinition(db: Db, siteId: string, experienceId: string) {
  ...
  if (!experience?.publishedVersionId) return null;
  ...
  return { experience, version, definition: version.definition as ChecklistExperienceDefinition };
}
