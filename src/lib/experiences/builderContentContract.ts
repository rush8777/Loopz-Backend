...
export function builderCssIsSafe(value: string): boolean {
  const css = value.replace(/\/\*[\s\S]*?\*\//g, "");
  if (BUILDER_UNSAFE_CSS.test(css)) return false;
...
export function builderHtmlIsSafe(value: string, allowSurveyInputs = false): boolean {
  if (/<\s*(script|style|iframe|object|embed|form|select|video|audio|source)\b|\son[a-z]+\s*=|javascript\s*:/i.test(value)) return false;
  for (const tagMatch of value.matchAll(/<\s*([a-z][\w-]*)\b([^>]*)>/gi)) {
    const tag = tagMatch[1].toLowerCase();
    if (!BUILDER_ALLOWED_TAGS.has(tag) && !(allowSurveyInputs && BUILDER_SURVEY_INPUT_TAGS.has(tag))) return false;
    ...
    if (tag === "input") {
      ...
      if (!builderInputTypeIsSafe(type)) return false;
    }
  }
  return true;
}
