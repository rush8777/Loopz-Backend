...
export function formatEvidence(evidence: BehavioralEventEvidence | null | undefined): string | null {
  if (!evidence) return null;
  const parts: string[] = [];
  if (evidence.durationMs != null) parts.push(`durationMs=${evidence.durationMs}`);
  if (evidence.distanceMoved != null) parts.push(`distanceMoved=${evidence.distanceMoved}px`);
  if (evidence.numberOfDirectionChanges != null) parts.push(`directionChanges=${evidence.numberOfDirectionChanges}`);
  if (evidence.sampleCount != null) parts.push(`sampleCount=${evidence.sampleCount}`);
  if (evidence.minDistanceToTarget != null) parts.push(`minDistToTarget=${evidence.minDistanceToTarget}px`);
  if (evidence.maxDistanceToTarget != null) parts.push(`maxDistToTarget=${evidence.maxDistanceToTarget}px`);
  if (evidence.windowMs != null) parts.push(`windowMs=${evidence.windowMs}`);
  if (evidence.targetIsClickable != null) parts.push(`targetIsClickable=${evidence.targetIsClickable}`);
...
export function formatVerboseAnnotation(opts: {
  ...
  count?: number | null;
  evidence?: BehavioralEventEvidence | null;
}): string | null {
  const bits: string[] = [];
  if (opts.durationMs != null) bits.push(`durationMs=${opts.durationMs}`);
  if (opts.count != null) bits.push(`count=${opts.count}`);
  ...
  const evidenceWithoutDuplicateDuration =
    opts.durationMs != null && opts.evidence?.durationMs === opts.durationMs
      ? { ...opts.evidence, durationMs: undefined }
      : opts.evidence;
  const evidenceText = formatEvidence(evidenceWithoutDuplicateDuration);
  if (evidenceText) bits.push(evidenceText);
  return bits.length > 0 ? bits.join(", ") : null;
}

function durationMsOf(event: BehavioralEvent): number | null {
  return "durationMs" in event && typeof event.durationMs === "number" ? event.durationMs : null;
}
function countOf(event: BehavioralEvent): number | null {
  return "count" in event && typeof event.count === "number" ? event.count : null;
}
function evidenceOf(event: BehavioralEvent): BehavioralEventEvidence | null {
 ...
 */
export function formatBehavioralEventVerbose(event: BehavioralEvent, token: string, indent = "    "): string {
