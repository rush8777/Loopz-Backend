...
export const dashboardFiltersSchema = z.object({
  ...
  if (until <= since) ctx.addIssue({ code: "custom", message: "until must be after since", path: ["until"] });
...
export const metricConfigurationSchema = z.object({
  ...
  if (conversion !== Boolean(value.conversion)) ctx.addIssue({ code: "custom", message: conversion ? "conversion definition is required" : "conversion is only valid for conversion metrics", path: ["conversion"] });
  if (conversion && value.events) ctx.addIssue({ code: "custom", message: "conversion metrics do not use event matching", path: ["events"] });
  if (value.metricId.startsWith("events.") && value.events?.match === "all") ctx.addIssue({ code: "custom", message: "All is not supported for occurrence metrics", path: ["events", "match"] });
  if ((value.mode === "breakdown") !== Boolean(value.breakdown)) ctx.addIssue({ code: "custom", message: value.mode === "breakdown" ? "breakdown is required" : "breakdown is only valid in breakdown mode", path: ["breakdown"] });
  ...
  if (!allowed.includes(value.visualization)) ctx.addIssue({ code: "custom", message: "visualization is not valid for this result mode", path: ["visualization"] });
  if (value.target && value.visualization === "table") ctx.addIssue({ code: "custom", message: "targets are not supported by tables", path: ["target"] });
  if (value.events?.match === "each" && value.breakdown) ctx.addIssue({ code: "custom", message: "Each cannot be combined with a second breakdown", path: ["breakdown"] });
});
...
}).superRefine((value, ctx) => { if (value.cardType !== value.configuration.kind) ctx.addIssue({ code: "custom", message: "cardType must match configuration kind", path: ["configuration", "kind"] }); });
