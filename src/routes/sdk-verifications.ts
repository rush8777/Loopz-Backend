...
function serializedChallenge(challenge: typeof sdkVerificationChallenges.$inferSelect, now = new Date()) {
  ...
  return {
    id: challenge.id,
    status: challenge.expiresAt.getTime() <= now.getTime() ? ("expired" as const) : ("pending" as const),
    expiresAt: challenge.expiresAt,
    detectedAt: null,
  };
}

export function registerSdkVerificationRoutes(app: FastifyInstance, db: Db) {
  app.post(
    "/orgs/:orgId/sites/:siteId/sdk-verifications",
  ...
  app.get(
    "/orgs/:orgId/sites/:siteId/sdk-verifications/:verificationId",
    { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] },
    async (request, reply) => {
      const { siteId, verificationId } = request.params as { siteId: string; verificationId: string };
      const [site] = await db.select().from(sites).where(eq(sites.id, siteId)).limit(1);
      if (!site || site.orgId !== request.membership!.orgId) {
        return reply.code(404).send({ error: "site_not_found" });
      }
      const [challenge] = await db
        ...
        .limit(1);
      if (!challenge) return reply.code(404).send({ error: "verification_not_found" });
      return reply.send({ verification: serializedChallenge(challenge) });
    }
  );
}

export function registerPublicSdkVerificationRoutes(app: FastifyInstance, db: Db) {
  app.post("/public/sites/:siteId/sdk-verifications/:verificationId/ack", async (request, reply) => {
    ...
    if (!site) return reply.code(404).send({ error: "verification_not_found" });
