...
async function loadTrackedUserInSite(db: Db, siteId: string, userId: string) {
  const [user] = await db
    .select()
    .from(trackedUsers)
...
async function propertiesFor(db: Db, trackedUserId: string) {
  const rows = await db
    .select()
...
export function registerTrackedUserRoutes(app: FastifyInstance, db: Db) {
  /**
  ...
  app.get(
    ...
    async (request, reply) => {
        ...
        const matches = await db
          ...
          .where(
            and(
              ...
              or(like(trackedUsers.externalUserId, term), like(trackedUserProperties.value, term))
            )
          );
      ...
      if (segmentId) {
        ...
        const members = [...await evaluateSegment(db, site.id, segment.definition as SegmentDefinition)];
        matchingIds = matchingIds ? matchingIds.filter((id) => members.includes(id)) : members;
      }
      const conditions = [eq(trackedUsers.siteId, site.id)];
      if (matchingIds) { if (matchingIds.length === 0) return reply.send({ users: [], total: 0, limit, offset }); conditions.push(inArray(trackedUsers.id, matchingIds)); }
      if (since) conditions.push(gte(trackedUsers.lastSeenAt, since)); if (until) conditions.push(lte(trackedUsers.lastSeenAt, until));
      const where = and(...conditions);

      const [{ total }] = await db.select({ total: sql<number>`count(*)` }).from(trackedUsers).where(where);
      ...
      if (userIds.length > 0) {
        const counts = await db
          ...
          .innerJoin(
            sessionEvents,
            and(eq(sessionEvents.anonymousId, trackedUserAliases.anonymousId), eq(sessionEvents.siteId, site.id))
          )
        ...
        for (const c of counts) sessionCounts.set(c.trackedUserId, c.sessionCount);
      }

      const propsByUser = new Map<string, Awaited<ReturnType<typeof propertiesFor>>>();
  ...
  app.get(
    "/orgs/:orgId/sites/:siteId/users/:userId",
    { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] },
    async (request, reply) => {
      const { siteId, userId } = request.params as { siteId: string; userId: string };
      const site = await loadSiteInOrg(db, siteId, request.membership!.orgId);
      if (!site) return reply.code(404).send({ error: "site_not_found" });
  ...
  app.get(
    "/orgs/:orgId/sites/:siteId/users/:userId/properties",
    { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] },
    async (request, reply) => {
      const { siteId, userId } = request.params as { siteId: string; userId: string };
      const site = await loadSiteInOrg(db, siteId, request.membership!.orgId);
