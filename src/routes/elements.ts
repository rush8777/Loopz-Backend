...
function serializeElementRow(row: typeof elementCatalog.$inferSelect) {
  return {
    id: row.id,
 ...
 */
export function registerElementRoutes(app: FastifyInstance, db: Db) {
  app.get(
    "/orgs/:orgId/sites/:siteId/elements",
  ...
  app.patch(
    "/orgs/:orgId/sites/:siteId/elements/:elementId",
    { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] },
    async (request, reply) => {
      const { siteId, elementId } = request.params as { siteId: string; elementId: string };
      const site = await loadSiteInOrg(db, siteId, request.membership!.orgId);
      if (!site) return reply.code(404).send({ error: "site_not_found" });
      ...
      const updates: Partial<typeof elementCatalog.$inferInsert> = {};
