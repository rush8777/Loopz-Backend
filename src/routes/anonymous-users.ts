...
export function registerAnonymousUserRoutes(app: FastifyInstance, db: Db) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      return reply.send({
        visitors: visitors.map((v) => ({ ...v, identityType: "anonymous" as const, isAnonymous: true })),
        total,
        limit,
        offset,
      });
   ...
   */
  app.get(
    "/orgs/:orgId/sites/:siteId/anonymous-users/:anonymousId",
    { preHandler: [authenticate, requireOrgRole(db, "VIEWER")] },
    async (request, reply) => {
      const { siteId, anonymousId } = request.params as { siteId: string; anonymousId: string };
      const site = await loadSiteInOrg(db, siteId, request.membership!.orgId);
      if (!site) return reply.code(404).send({ error: "site_not_found" });

      const state = await resolveAnonymousIdentityState(db, site.id, anonymousId);
      if (!state.exists) return reply.code(404).send({ error: "anonymous_visitor_not_found" });
