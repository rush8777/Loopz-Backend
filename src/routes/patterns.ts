...
export function registerPatternRoutes(app: FastifyInstance, db: Db) {
  app.post(
    "/orgs/:orgId/sites/:siteId/patterns",
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const matchCountRows = await db
        .select({ patternId: patternMatches.patternId, count: sql<number>`count(*)` })
        .from(patternMatches)
  ...
  app.patch(
    "/orgs/:orgId/sites/:siteId/patterns/:patternId",
    { preHandler: [authenticate, requireOrgRole(db, "ADMIN")] },
    async (request, reply) => {
      const { siteId, patternId } = request.params as { siteId: string; patternId: string };
      const site = await loadSiteInOrg(db, siteId, request.membership!.orgId);
      if (!site) return reply.code(404).send({ error: "site_not_found" });

      const [existing] = await db.select().from(patterns).where(eq(patterns.id, patternId)).limit(1);
      if (!existing || existing.siteId !== site.id) return reply.code(404).send({ error: "pattern_not_found" });
...
function serializePattern(row: typeof patterns.$inferSelect) {
  return {
    id: row.id,
    siteId: row.siteId,
    name: row.name,
